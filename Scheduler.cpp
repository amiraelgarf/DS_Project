...
void Scheduler::FileLoad(string filename)
{
	...
	else
	{
		...
		totalNumberofProcessesn = totalNumberOfProcesses;
...
{
	int Temp = totalNumberofProcessesn;
...
void Scheduler::ProcessorOverheating()
{
	...
	if (listOfProcessors[rmp]->isReadyEmpty() == false && totalNumberOfProcessors!=1)
	{
		...
		P = listOfProcessors[rmp]->GetRunningProcess();
		if (P != NULL)
		{
			GetLongest_ShortestReady();
			listOfProcessors[ShortestQueueIndex]->AddToRDYQueue(P);
			listOfProcessors[rmp]->ToRUN_State(P);
			numberOfProcessInRun--;
		}
